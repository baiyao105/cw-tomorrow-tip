...
from qfluentwidgets import PrimaryPushButton, MessageBox, TimePicker, LineEdit, SpinBox  # 导入qfluentwidgets控件
...
class Plugin(PluginBase):  # 插件类
    ...
    def show_tomorrow_courses(self, tomorrow_weekday, is_test=False):
        ...
        if not os.path.exists(schedule_path):
            with open(self.log_path, 'a', encoding='utf-8') as f:
                f.write(f'[{dt.datetime.now().strftime("%Y-%m-%d %H:%M:%S")}] 无法获取明日课程信息：课程表文件不存在 ({schedule_path})\n')
            return
        ...
        try:
            ...
            for item_name, item_time in sorted(timeline.items()):
                if item_name.startswith('a'):
                    ...
                    try:
                        ...
                        if class_count < len(schedule):
                            ...
                            if course_name != '未添加' and course_name != '暂无课程':
                                ...
                                if course_name not in excluded_courses:
                                    ...
                                    with open(self.log_path, 'a', encoding='utf-8') as f:
                                        f.write(f'[{dt.datetime.now().strftime("%Y-%m-%d %H:%M:%S")}] 课程 "{course_name}" 在排除列表中，不显示\n')
                        class_count += 1
            ...
            with open(self.log_path, 'a', encoding='utf-8') as f:
                f.write(f'[{dt.datetime.now().strftime("%Y-%m-%d %H:%M:%S")}] 发送通知，显示时间设置为: {notification_duration}毫秒（{notification_duration//1000}秒）\n')
...
class Settings(SettingsBase):
    def __init__(self, plugin_path, parent=None):
        ...
        if hasattr(self, 'cw_contexts') and self.cw_contexts:
            ...
            if schedule_name == 'backup.json':
                ...
                # 显示警告信息
                msg_box = MessageBox('警告', '当前使用可能是调休课表(backup.json)，通知已被临时禁用。\n请恢复原课表后再使用通知功能。', self)
    ...
    def test_notification(self):
        ...
        if hasattr(self, 'is_backup_schedule') and self.is_backup_schedule:
            msg_box = MessageBox('测试通知', '当前使用可能是调休课表(backup.json)，通知已被临时禁用。\n请恢复原课表后再使用通知功能。', self)
            msg_box.yesButton.setText('确定')
            msg_box.cancelButton.setVisible(False)
            msg_box.exec()
            return
        ...
        try:
            ...
            # 创建临时插件
            plugin_instance = Plugin(self.cw_contexts if hasattr(self, 'cw_contexts') else {}, self.method if hasattr(self, 'method') else None)
